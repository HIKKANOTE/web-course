...
procedure TTestForm1.LoadQuestion(Index: integer);
begin
 if QuestionText[Index]<>'' then// если выбранный вопрос существует то загружаем его
  begin
  ...
  TestForm3.Label3.Caption:= IntToStr(rez);
  TestForm3.Label4.Caption:= floatToStr(r);
  TestForm3.ShowModal;
  end;
end;
...
procedure TTestForm1.LoadTest(Index: integer);
var
  s: Tstrings;
  j, h: Integer;
begin
  ...
  s.LoadFromFile(ExtractFilePath(application.ExeName)+TestName+inttostr(Index)+'.txt'); // загружаем
    ...
    begin
    TestForm1.Caption:=s.Strings[0]; // название теста
...
procedure TTestForm1.TestBoxCloseUp(Sender: TObject);
begin
  ball:=0;
  now:=0;
